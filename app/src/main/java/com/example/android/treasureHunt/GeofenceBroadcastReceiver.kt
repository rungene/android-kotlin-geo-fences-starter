...
//In the case that there is an error, you will want to understand what went wrong. Log the error
...
//Check if the geofenceTransition type is ENTER.
 ...
 //If the triggeringGeofences array is not empty, set the fenceID to the first geofence’s requestId.
                // We would only have one geofence active at a time, so if the array is non-empty
                // then there would only be one for us to interact with. If the array is empty,
...
//Check that the geofence is consistent with the constants listed in GeofenceUtil.kt. If not,
...
//the user has found a valid geofence. Send a notification telling them
